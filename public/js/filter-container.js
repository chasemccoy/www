class FilterContainer extends HTMLElement {
  ...
  elementIsValid(element, attributeName, values) {
    ...
    let haystack = (element.getAttribute(attributeName) || '').split(
      this.valueDelimiter
    )
  ...
  _renderResultCount(count) {
    ...
    this.resultsCounter.innerText = `${count} ${
      count !== 1 ? labels[1] : labels[0]
    }`
  }
  ...
  updateUrl(key, values) {
    ...
    if (keyParamsStr !== valuesStr) {
      ...
      history.replaceState(
        {},
        '',
        `${baseUrl}${params.toString().length > 0 ? `?${params}` : ''}`
      )
    }
  }
}
